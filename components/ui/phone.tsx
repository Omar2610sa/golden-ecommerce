  ...
  return (
    ...
    >
      <BasePhoneInput.default
        className={cn(
          'flex h-11 w-full items-center rounded-lg border border-input bg-transparent overflow-hidden transition-colors',
          'has-[input:focus-visible]:border-primary has-[input:focus-visible]:ring-3 has-[input:focus-visible]:ring-primary/50',
          props['aria-invalid'] &&
          'border-destructive ring-destructive/50 [&_*[data-slot=combobox-trigger]]:border-destructive',
          className
        )}
...
const InputComponent = ({ className, ...props }: ComponentProps<typeof Input>) => {
  ...
  return (
    <Input
      className={cn(
        'h-full w-full min-w-0 flex-1 text-end rounded-none border-0 bg-transparent px-2.5 py-1 text-base shadow-none outline-none ring-0',
      ...
      <ComboboxTrigger
        render={
          <Button
            ...
            size={variant}
            className={cn(
              'flex h-full shrink-0 items-center gap-1 rounded-none border-0 border-e border-gray-400 bg-muted/40 px-2.5 py-0 leading-none hover:bg-muted/60 focus:z-10 aria-pressed:bg-transparent',
          ...
          >
            ...
            <span className='text-sm'>
              {selectedCountry ? `+${BasePhoneInput.getCountryCallingCode(selectedCountry)}` : ''}
            </span>
